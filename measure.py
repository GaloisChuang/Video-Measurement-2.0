...
def list_mask_paths(mask_root: Optional[Path], instance_id: str, T: int,
                    frame_names: Optional[List[str]] = None) -> List[Optional[Path]]:
    ...
    for t in range(T):
        ...
        if frame_names and t < len(frame_names):
            fn = frame_names[t]
            p1 = mask_root / str(instance_id) / fn  # exact filename match (e.g., 000.png)
            if p1.exists():
                cand = p1
        if cand is None:
            # Fallbacks: zero-padded conventions
...
def compute_metrics(orig_frames: List[np.ndarray], edit_frames: List[np.ndarray], spec: Dict, device: str = "cpu",
    ...
    src_mat = torch.stack([id2src[iid] for iid in valid_ids], dim=0)        # [N,D]

    # Similarity matrix S[i,j] = sim(inst_i, target_text_j)
    S = img_mat @ tgt_mat.T

    # CIA: top-1 along rows equals diagonal index
    ...
    for i in range(len(valid_ids)):
        sim_t = F.cosine_similarity(img_mat[i], tgt_mat[i], dim=0).item()
        sim_s = F.cosine_similarity(img_mat[i], src_mat[i], dim=0).item()
        ia_list.append(1.0 if sim_t > sim_s else 0.0)
    ia = float(np.mean(ia_list)) if len(ia_list) else float("nan")
